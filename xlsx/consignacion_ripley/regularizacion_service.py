...
def regularizacion_intek(path_liquidacion, sku_intek_df):
    ...
    for col_idx, titulo in nuevas_columnas.items():
        ...
        celda_titulo.fill = styles["fill_naranja"]
        celda_titulo.font = styles["font_blanco"]
    ...
    for fila in range(7, ultima_fila + 1):
        try:
            ...
            costo = float(sku_intek_df.loc[sku_ripley, "VALOR VENTA"])
            ...
            detalle_sheet.cell(row=fila, column=17).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
            detalle_sheet.cell(row=fila, column=18).value = f"=Q{fila}*G{fila}" # Ejemplo para Col R
            detalle_sheet.cell(row=fila, column=18).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
            ...
            if fecha_venta >= periodo[0] and fecha_venta <= periodo[1]:
                ...
                detalle_sheet.cell(row=fila, column=19).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
            detalle_sheet.cell(row=fila, column=20).value = f"=S{fila}*G{fila}"  # Rec Total
            detalle_sheet.cell(row=fila, column=20).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
            detalle_sheet.cell(row=fila, column=21).value = f"=R{fila}-T{fila}"  # Pago Neto (Deben Pagar)

            
            detalle_sheet.cell(row=fila, column=21).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
            ...
            detalle_sheet.cell(row=fila, column=22).number_format = styles["formato_moneda"]  # Formato de número con 2 decimales y separador de miles
